...
test("shows the version stamp and entity count", async ({ page }) => {
  ...
  await expect(page.getByTestId("text-app-version")).toContainText("v3.33.1.0");
  await expect(page.getByTestId("text-app-version")).toContainText("data 0.34.1.0");
  await expect(page.getByTestId("panel-footer-root")).toContainText("1,765 entities");
});
