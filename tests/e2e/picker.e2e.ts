...
test("shows the version stamp and entity count", async ({ page }) => {
  ...
  await expect(page.getByTestId("text-app-version")).toContainText("v3.29.0.0");
  await expect(page.getByTestId("text-app-version")).toContainText("data 0.30.0.0");
  await expect(page.getByTestId("panel-footer-root")).toContainText("1,705 entities");
});
